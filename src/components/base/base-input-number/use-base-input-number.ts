...
function snapToIntStepBigInt(value: bigint, step: number, min?: number): bigint {
...
function applyMinMaxToDigits(digits: string, min: number | undefined, max: number | undefined): string {
  if (digits === '') {
    return ''
  }
  let v = BigInt(digits)
